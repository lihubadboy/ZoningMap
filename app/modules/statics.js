    ...
    _requireStaticDatas: function(id) {
      ...
      _self.ajaxUtil.query(_self.options.OprUrls.calculator.queryUrl, '1=1', function(respons) {
        if (respons.result) {
          ...
          if(id == 'pointmap')
            _self._constructPointVisualMap(_self.points, 'mapchart');
          else if(id == 'heatmap')
            _self._constructPointHeatMap(_self.hpoints, 'mapchart');
          else if(id == 'staticmap')
            _self._constructClusterMap(_self.markers, 'mapchart');
        } else {
          _self._raiseMessage('获取用量数失败！');
        }
        $('.left').removeClass('loading-light');
